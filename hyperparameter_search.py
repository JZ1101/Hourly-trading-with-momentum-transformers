...
def grid_search_with_backtest(
    ...
    test_size=0.2,
    val_size=0.2,
    base_experiment_dir='experiments/grid_search',
    max_epochs=100,
    backtest_params={
        'commission': 0.001, 
    ...
    experiment_dir = os.path.join(base_experiment_dir, f'grid_search_{timestamp}')
    ...
    # BACKTESTING PHASE - Now that all models are trained, we can run backtests
    if len(results) > 0:
        if backtest_all:
    ...
    results_df = pd.DataFrame(results)
    results_df.to_csv(os.path.join(experiment_dir, 'grid_search_results_final.csv'), index=False)
